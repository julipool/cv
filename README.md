# Julio Diego Barrado Fernández

### Senior Salesforce Developer

📍 Seville, Spain · [LinkedIn](https://www.linkedin.com/in/juliodiegobarradofernandez/) · [Trailblazer Profile](https://www.salesforce.com/trailblazer/julipool)

![Salesforce](https://img.shields.io/badge/Salesforce-00A1E0?style=flat&logo=salesforce&logoColor=white)
![Apex](https://img.shields.io/badge/Apex-1798c1?style=flat)
![LWC](https://img.shields.io/badge/Lightning%20Web%20Components-blue?style=flat)
![Talend](https://img.shields.io/badge/Talend-FF6D70?style=flat)
![Trailhead Ranger](https://img.shields.io/badge/Trailhead-Ranger-00A1E0?style=flat&logo=salesforce&logoColor=white)
![Salesforce MVP](https://img.shields.io/badge/Salesforce-MVP-orange?style=flat)

---

## About

> Working in the Cloud, from my little office in Seville (Spain) to the world! To and from Bristol (UK) every now and then and making the wonders of working in the cloud a reality! I'm one of the leaders of the Salesforce Developer Group in Seville. I'm also one of the founders and co-organiser of dreamOlé, the community-led dreamin' conference in Spain. And I've now been part of the Salesforce MVP (Most Valuable Professionals) program for a couple of years (2019–2020), very excited to add my contribution to the Salesforce Trailblazer Community. World traveller, beginner skater, DIY lover, learning how to sew is one of my last hobbies.

## Professional Summary

Senior Salesforce Developer with 14+ years on the platform (19 years in IT overall), currently at Desynit, a UK-based Salesforce Partner. Deep expertise across the full Salesforce stack — Apex, LWC, Aura, Visualforce and Flow — with a particular strength in data integration and data quality using Talend, and DevOps practices such as CircleCI-driven CI/CD. Six active Salesforce certifications spanning Platform, JavaScript, Agentforce and Data 360.

## Experience

### Senior Salesforce Developer — Desynit Limited
*March 2012 – Present · Bristol, UK (remote from Seville, Spain)*

- Full-stack Salesforce development across Apex, SOQL/SOSL, Visualforce, Aura, Lightning Web Components and Flow
- Built third-party integrations with a specialism in Data Integration and Data Quality, using ETL tools to deliver data loads and quality reporting into Salesforce and external systems
- Experienced working with Agile methodologies and a strong TDD focus; recently incorporated AI into my toolset as part of a continuous learning approach to work
- Worked across all aspects of consultancy — support, technical implementation, client-facing work and specifications gathering — typical of operating within a small company, while contributing to projects of every size

**Key client engagements**

- **Ovo Energy** (independent UK energy supplier) — *Mar 2020 – Aug 2025 (5+ years)* — long-term embedded engagement owning DevOps on CircleCI for CI/CD and building integrations with Kafka-based services, while heavily involved in LWC and JavaScript customisation for front-end UX for agents, as well as backend processes and integrations
- **Dojo** (payment provider) — *Jan 2026 – Jul 2026* — improved existing Flow implementations: diagnosed and fixed issues, simplified and optimised Flow logic for maintainability and performance

### Earlier Experience (Pre-Salesforce)

| Role | Company | Dates | Location |
|---|---|---|---|
| IBM Maximo Consultant | ECCS Limited | May 2010 – Feb 2012 | Bath, UK |
| Analyst | Accenture | Sep 2008 – Dec 2009 | Madrid, Spain |
| Junior Developer | Sadiel | Apr 2008 – Aug 2008 | Seville, Spain |
| Roaming Databases Manager (Intern) | T-Mobile Czech Republic | Sep 2007 – Dec 2007 | Prague, Czechia |

Enterprise Asset Management consulting (incl. a long-term Rolls-Royce engagement), Java-based systems integration for BBVA, and early web/telecom internships — full details on request.

## Salesforce Certifications

| Certification | Issued |
|---|---|
| Salesforce Certified Data 360 Consultant | Aug 2026 |
| Salesforce Certified Agentforce Specialist | Dec 2025 |
| Salesforce Certified JavaScript Developer | Sep 2021 |
| Salesforce Certified Platform Developer II | Jan 2020 |
| Salesforce Certified Platform Developer I | Oct 2017 |
| Salesforce Certified Platform App Builder | Apr 2017 |
| Salesforce Certified Force.com Developer *(retired)* | Jul 2013 |

## Community

- Salesforce MVP (Most Valuable Professional), 2019 – 2020
- Leader, Salesforce Developer Group Seville
- Co-founder & co-organiser, [dreamOlé](https://dreamole.com) — community-led Salesforce "Dreamin'" conference, Spain
- Conference speaker: French Touch Dreamin', North Africa Dreamin', Czech Dreamin', YeurDreamin' and other community-led events across Europe and Morocco — [watch the FTD19 talk](https://www.youtube.com/watch?v=fSD5qRIGxG4)
- Attended Dreamforce six times (DF14, DF15, DF17, DF18, DF19, DF22), including multiple appearances as a speaker — [watch the DF17 talk](https://www.youtube.com/watch?v=KOEF_eS_w6M)

## Technical Skills

**Salesforce Platform:** Apex · Lightning Web Components (LWC) · Aura · Visualforce · Flow · SOQL/SOSL · Agentforce · Data 360
**Integration & Data:** Talend (Data Integration, Data Quality, ESB) · ETL · Kafka · Web Services · XML
**DevOps & Practices:** CircleCI · Agile · TDD · peer programming

## Education

**Telecommunications Engineering** — Universidad de Sevilla, Spain (2001 – 2008)

## Languages

English (Full professional proficiency) · Spanish (Native)

---

📫 Get in touch via [LinkedIn](https://www.linkedin.com/in/juliodiegobarradofernandez/)
